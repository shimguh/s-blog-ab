...
#var
blog_a=s-blog-a.internal
blog_b=s-blog.internal
...
# run
figlet run
sudo docker run -d --name $blog_a --network=abc -p 8001:80 blog-a
sudo docker run -d --name $blog_b --network=abc -p 8002:80 blog-b
